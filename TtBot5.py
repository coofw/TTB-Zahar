...
def get_user_main_keyboard() -> types.ReplyKeyboardMarkup:
    ...
    keyboard.add(types.KeyboardButton("🍎На ios(beta)"))  # <--- ДОБАВЛЕНА КНОПКА
    return keyboard
...
# ОБРАБОТКА НАЖАТИЯ НА КНОПКУ iOS
@bot.message_handler(func=lambda message: message.text == "🍎На ios(beta)")
def handle_ios_button(message):
